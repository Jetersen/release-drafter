...
module.exports.findReleases = async ({ log, github, context }) => {
  let releases = await github.paginate(
    github.repos.listReleases.endpoint.merge({
      ...context.repo(),
      per_page: 100,
    })
  )
  ...
  if (draftRelease) {
    log.info(`Draft release: ${draftRelease.tag_name}`)
  } else {
    log.info(`No draft release found`)
  }

  if (lastRelease) {
    log.info(`Last release: ${lastRelease.tag_name}`)
  } else {
    log.info(`No last release found`)
  }
...
module.exports.generateReleaseInfo = ({
  ...
  let { version, tag, name, template: body, prerelease, shouldDraft } = config
...
module.exports.createRelease = ({ github, context, releaseInfo }) => {
